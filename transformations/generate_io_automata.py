...
from models.io_automaton import IOAutomaton, Transition
...
class GenerateIOAutomata:
    ...
    def io_automata(self):
        _io_automata: Dict[str, IOAutomaton] = {}
        for obj, behaviors in self._input.items():
            ...
            _io_automata.update(
                {obj: IOAutomaton(states=list(_states), transitions=_transitions)}
            )
        _io_automata_final: Dict[str, IOAutomaton] = {}
        for obj, automat in _io_automata.items():
            ...
            _io_automata_final.update(
                {obj: IOAutomaton(states=_states, transitions=_transitions)}
            )
